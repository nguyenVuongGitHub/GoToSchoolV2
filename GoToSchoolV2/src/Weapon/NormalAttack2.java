...
public class NormalAttack2 extends BaseSkill{
    public static int TIME_COUNT_DOWN_ATTACK = 1;
    public static final int TIME_REDUCE = 1;

    public NormalAttack2(GameState gs) {
        super(gs);
        init();
    }
    public void getImage() {
        try {
            BufferedImage largeImage = ImageIO.read(Objects.requireNonNull(getClass().getResourceAsStream("/weapon/purpleBall.png")));
            int x = 0;
            int y = 0;
            int scale = 2;
    ...
    public void init() {
        type = TYPE.WEAPON;
        typeSkill.typeAttack = ATTACK_SKILL.PURPLE;
        distance = 50;
        damage = 50;
        speed = 25;
